...
public sealed class LoginModel(SignInManager<ApplicationUser> signInManager, UserManager<ApplicationUser> users, OrganizationService organizations) : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync(CancellationToken cancellationToken)
    {
        ...
        if (user is null) { ModelState.AddModelError(string.Empty, "Invalid sign-in attempt."); return Page(); }
        var result = await signInManager.CheckPasswordSignInAsync(user, Input.Password, lockoutOnFailure: true);
        if (!result.Succeeded) { ModelState.AddModelError(string.Empty, "Invalid sign-in attempt."); return Page(); }
        var memberships = await organizations.GetForUserAsync(user.Id, cancellationToken);
        if (memberships.Count == 0) { ModelState.AddModelError(string.Empty, "This account is not a member of an organization."); return Page(); }
